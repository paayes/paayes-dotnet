namespace Paayes
{
    ...
    /// <summary>
    /// Standard client to make requests to Paayes's API, using
    ...
    public class SystemNetHttpClient : IHttpClient
    {
        ...
        private const string PaayesNetTargetFramework =
#if NET5_0
            "net5.0"
#elif NETSTANDARD2_1
            "netstandard2.1"
        ...
        private string paayesClientUserAgentString;

        private string userAgentString;

        static SystemNetHttpClient()
        {
            // Enable support for TLS 1.2, as Paayes's API requires it. This should only be
        ...
        /// for plugins that wish to identify themselves with Paayes.
        /// </param>
            ...
            bool enableTelemetry = true)
        {
            ...
            this.paayesClientUserAgentString = this.BuildPaayesClientUserAgentString();
        ...
        {
            // We set the User-Agent and X-Paayes-Client-User-Agent headers in each request
        ...
        public async Task<PaayesResponse> MakeRequestAsync(
        ...
        public async Task<PaayesStreamedResponse> MakeStreamingRequestAsync(
            PaayesRequest request,
            CancellationToken cancellationToken = default)
        {
            var (response, retries) = await this.SendHttpRequest(request, cancellationToken).ConfigureAwait(false);

            return new PaayesStreamedResponse(
                response.StatusCode,
                response.Headers,
                await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                this.requestTelemetry.MaybeAddTelemetryHeader(request.PaayesHeaders);
            }
        ...
        private string BuildPaayesClientUserAgentString()
        {
            var values = new Dictionary<string, object>
            {
                { "bindings_version", PaayesConfiguration.PaayesNetVersion },
                { "lang", ".net" },
                { "publisher", "paayes" },
                { "paayes_net_target_framework", PaayesNetTargetFramework },
            };
            ...
            // See https://github.com/paayes/paayes-dotnet/issues/1986 for context.
            try
            {
        ...
        {
            var userAgent = $"Paayes/v1 .NetBindings/{PaayesConfiguration.PaayesNetVersion}";
            ...
            HttpHeaders headers)
        {
            ...
            if (headers != null && headers.Contains("Paayes-Should-Retry"))
            {
                var value = headers.GetValues("Paayes-Should-Retry").First();
        ...
        private System.Net.Http.HttpRequestMessage BuildRequestMessage(PaayesRequest request)
        {
            ...
            // Custom headers
            requestMessage.Headers.Add("X-Paayes-Client-User-Agent", this.paayesClientUserAgentString);
            foreach (var header in request.PaayesHeaders)
            {
