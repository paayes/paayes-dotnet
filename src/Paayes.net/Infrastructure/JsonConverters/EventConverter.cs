namespace Paayes.Infrastructure
{
    ...
    /// formatted with. In the general case, Paayes.net uses a specific API version
    /// (<see cref="PaayesConfiguration.ApiVersion"/>), but events are a special case: when
    /// receiving an event object via a webhook notification, the object will be formatted with
    /// the Paayes account's default API version which may be different than Paayes.net's API
    /// version.
    /// </summary>
