namespace Paayes
{
    ...
    /// The type of <see cref="IPaayesEntity"/> that this service returns.
    /// </typeparam>
    public abstract class Service<TEntityReturned>
        where TEntityReturned : IPaayesEntity
    {
        private IPaayesClient client;
        ...
        /// <see cref="PaayesConfiguration.PaayesClient"/> is used instead.
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        public IPaayesClient Client
        {
            get => this.client ?? PaayesConfiguration.PaayesClient;
        ...
        protected PaayesList<TEntityReturned> ListEntities(
            ListOptions options,
            RequestOptions requestOptions)
        {
            return this.Request<PaayesList<TEntityReturned>>(
                HttpMethod.Get,
                this.ClassUrl(),
                options,
                requestOptions);
        ...
        protected Task<PaayesList<TEntityReturned>> ListEntitiesAsync(
            ListOptions options,
            RequestOptions requestOptions,
            CancellationToken cancellationToken)
        {
            return this.RequestAsync<PaayesList<TEntityReturned>>(
                HttpMethod.Get,
                this.ClassUrl(),
                options,
                requestOptions,
        ...
        protected IEnumerable<T> ListRequestAutoPagingSync<T>(
            ...
            where T : IPaayesEntity
        {
            var page = this.Request<PaayesList<T>>(
                HttpMethod.Get,
                url,
                options,
                requestOptions);
            ...
            while (true)
            {
                ...
                {
                    // Elements in `PaayesList` instances are decoded by `PaayesObjectConverter`,
                ...
                page = this.Request<PaayesList<T>>(
                    HttpMethod.Get,
                    url,
                    options,
                    requestOptions);
        ...
        protected async IAsyncEnumerable<T> ListRequestAutoPagingAsync<T>(
            ...
            where T : IPaayesEntity
        {
            var page = await this.RequestAsync<PaayesList<T>>(
                HttpMethod.Get,
                url,
                options,
                requestOptions,
            ...
            while (true)
            {
                ...
                page = await this.RequestAsync<PaayesList<T>>(
                    HttpMethod.Get,
                    url,
                    options,
                    requestOptions,
        ...
        private static bool IsPaayesList<T>()
        {
            var typeInfo = typeof(T).GetTypeInfo();
            return typeInfo.IsGenericType
                && typeInfo.GetGenericTypeDefinition() == typeof(PaayesList<>);
        }
    }
}
