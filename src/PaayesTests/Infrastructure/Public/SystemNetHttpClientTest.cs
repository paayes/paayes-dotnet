namespace PaayesTests
{
    ...
    public class SystemNetHttpClientTest : BasePaayesTest
    {
        ...
        public async Task MakeStreamingRequestAsync()
        {
            ...
            var response = await streamedResponse.ToPaayesResponseAsync();
            Assert.Equal("Hello world!", response.Content);
        }
        ...
        private bool VerifyHeaders(HttpRequestHeaders headers)
        {
            ...
            var appInfo = JObject.Parse(headers.GetValues("X-Paayes-Client-User-Agent").First())["application"];
