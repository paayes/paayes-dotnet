namespace PaayesTests
{
    ...
    public class OAuthTokenServiceTest : BasePaayesTest
    {
        ...
        {
            this.service = new OAuthTokenService(this.PaayesClient);
        ...
        public void AuthorizeUrl_Standard()
        {
            var options = new OAuthAuthorizeUrlOptions
            {
                ...
                Scope = "read_write",
                State = "csrf_token",
                PaayesUser = new OAuthAuthorizeUrlPaayesUserOptions
                {
            ...
            Assert.Equal("connect.paayes.com", uri.Host);
            ...
            Assert.Equal("test@example.com", query["paayes_user[email]"]);
            Assert.Equal("https://example.com/profile/test", query["paayes_user[url]"]);
            Assert.Equal("US", query["paayes_user[country]"]);
        }
        ...
        [Fact]
        public void AuthorizeUrl_UseClientIdFromPaayesClientIfNotProvided()
        {
            var options = new OAuthAuthorizeUrlOptions { Scope = "read_write" };
            ...
            var query = this.ParseQueryString(uri.Query);
            Assert.Equal(this.PaayesClient.ClientId, query["client_id"]);
        }

        [Fact]
        public void Create()
        {
            // paayes-mock doesn't support OAuth endpoints, so stub the request
            ...
            Assert.Equal("acct_test_token", oauthToken.PaayesUserId);
        }
        ...
        public void Create_Error()
        {
            ...
            Assert.Equal("invalid_grant", exception.PaayesError.Error);
            Assert.Equal("Authorization code does not exist: ac_123", exception.PaayesError.ErrorDescription);
        }
        ...
        {
            // paayes-mock doesn't support OAuth endpoints, so stub the request
            var json = $"{{\"paayes_user_id\": \"{AccountId}\"}}";
            ...
            Assert.Equal(AccountId, deauth.PaayesUserId);
        }
        ...
        public void Deauthorize_Error()
        {
            ...
            Assert.NotNull(exception.PaayesError);
            Assert.Equal("invalid_client", exception.PaayesError.Error);
            Assert.Equal("No such application: ca_123", exception.PaayesError.ErrorDescription);
        }
