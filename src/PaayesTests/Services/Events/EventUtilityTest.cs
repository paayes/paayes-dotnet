namespace PaayesTests
{
    ...
    public class EventUtilityTest : BasePaayesTest
    {
            ...
            : base()
        {
            ...
            //   (echo -n "1533204620." && cat src/PaayesTests/Resources/event_test_signature.json) | openssl sha256 -hmac "webhook_secret"
        ...
        public void RejectIncorrectSignature()
        {
            ...
            Assert.Equal("The signature for the webhook is not present in the Paayes-Signature header.", exception.Message);
        }
        ...
        public void AcceptsExpectedApiVersion()
        {
            ...
            evt.ApiVersion = PaayesConfiguration.ApiVersion;
            var serialized = evt.ToJson();

            evt = EventUtility.ParseEvent(serialized);
            Assert.Equal(PaayesConfiguration.ApiVersion, evt.ApiVersion);
        }
