namespace PaayesTests
{
    ...
    public class TelemetryTest : BasePaayesTest
    {
            ...
            this.MockHttpClientFixture.MockHandler.Protected()
                .Verify(
                    ...
                    ItExpr.Is<HttpRequestMessage>(m =>
                        !m.Headers.Contains("X-Paayes-Client-Telemetry")),
        ...
        public void NoTelemetryWhenDisabled()
        {
            ...
            var paayesClient = new PaayesClient("sk_test_123", httpClient: httpClient);

            mockHandler.Reset();
            var fakeServer = FakeServer.ForMockHandler(mockHandler);
            fakeServer.Delay = TimeSpan.FromMilliseconds(20);

            var service = new BalanceService(paayesClient);
            service.Get();
            fakeServer.Delay = TimeSpan.FromMilliseconds(40);
            service.Get();
            service.Get();
        ...
        {
            if (!headers.Contains("X-Paayes-Client-Telemetry"))
            {
                return false;
            }

            var payload = headers.GetValues("X-Paayes-Client-Telemetry").First();
